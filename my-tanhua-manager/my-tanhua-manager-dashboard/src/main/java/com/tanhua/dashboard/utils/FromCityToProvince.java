package com.tanhua.dashboard.utils;
...
public class FromCityToProvince {
    ...
    public static List<String[]> AllProvinceList() {
        ArrayList<String[]> allProvinceList = new ArrayList<String[]>();
        String ss = "华东地区包括：山东,江苏,江西,浙江,安徽,福建,上海\n" +
                "华西地区包括：新疆,青海,西藏,四川,重庆,云南,宁夏,陕西,甘肃,内蒙古\n" +
                "华南地区包括：广东,广西,海南,香港,澳门,台湾\n" +
                "华北地区包括：北京,天津,河北,山西,内蒙古,黑龙江,吉林,辽宁\n" +
                "华中地区包括：湖北,湖南,河南";
    ...
    public static String findRegion(String province) {
        for (int i = 0; i < AllProvinceList().size(); i++) {
            for (int j = 0; j < AllProvinceList().get(i).length; j++) {
                if (AllProvinceList().get(i)[j].equals(province)) {
                    String regionName = allRegionList().get(i);
                    return regionName;
                }
            }
        }
